document.addEventListener("DOMContentLoaded", () => {
  ...
  // PERBAIKAN: Inisialisasi modal dengan opsi { focus: false }
  const createModal = new bootstrap.Modal(createModalEl, { focus: false });
  const detailModal = new bootstrap.Modal(detailModalEl, { focus: false });
    ...
    try {
      ...
      if (response.data.data.length > 0) {
        response.data.data.forEach((item) => {
          ...
          let tipeClass =
            item.tipe_permintaan === "pembelian" ? "bg-primary" : "bg-info";
        ...
        html =
          '<tr><td colspan="7" class="text-center">Belum ada data permintaan.</td></tr>';
      }
  ...
  form.addEventListener("submit", async (e) => {
    ...
    document.querySelectorAll(".item-row").forEach((row) => {
      ...
      if (isCustom) {
        itemData.nama_barang_custom = row.querySelector(
          ".item-barang-custom"
        ).value;
  ...
  tableBody.addEventListener("click", async (e) => {
    ...
    try {
      ...
      let detailHtml = `
                ...
                <p><strong>Tipe:</strong> <span class="badge bg-primary">${
                  header.tipe_permintaan
                }</span></p>
      ...
      items.forEach((item) => {
        ...
        const itemName = item.id_barang
          ? item.nama_barang
          : `<i>${item.nama_barang_custom} (Barang Baru)</i>`;
        detailHtml += `
                    <tr>
                        <td>${itemName}</td>
      ...
      if (header.status_permintaan !== "Diajukan") {
        detailHtml += `
                    ...
                    <p><strong>Diproses oleh:</strong> ${
                      header.nama_penyetuju || "-"
                    }</p>
                    <p><strong>Tanggal Proses:</strong> ${new Date(
                      header.tanggal_diproses
                    ).toLocaleString("id-ID")}</p>
                    <p><strong>Catatan Pimpinan:</strong><br>${
                      header.catatan_penyetuju || "-"
                    }</p>
                `;
      }
  ...
  document
    ...
    .addEventListener("click", async (e) => {
      ...
      if (action === "approve") {
        document
          .querySelectorAll("#detail-item-list .approved-qty")
          .forEach((input) => {
            itemsToProcess.push({
      ...
      const { value: catatan } = await Swal.fire({
        ...
        // PERBAIKAN: Fokuskan ke input saat SweetAlert terbuka
        didOpen: () => {
